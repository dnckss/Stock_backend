...
# 워밍 전용 lookback — 사용자가 90일 default 를 그대로 쓰면 매 사이클 22000+행 페이지네이션
# (분석 사이클 동안 다른 endpoint 막힘). 워밍은 최근 30일만으로 충분.
# 사용자가 직접 호출 시에는 BACKTEST_DEFAULT_LOOKBACK_DAYS(90) 그대로 사용.
BACKTEST_WARMUP_LOOKBACK_DAYS = int(os.getenv("BACKTEST_WARMUP_LOOKBACK_DAYS", "30"))
